...
numHiddenUnits1 = 10;  % number of hidden units of the first LSTM layer
numHiddenUnits2 = 10;  % number of hidden units of the second LSTM layer
...
best_Acc_mean = Inf;
best_Acc_std = Inf;
best_numHiddenUnits1 = 0;
best_numHiddenUnits2 = 0;
best_numFilters = 0;
best_learnRate = 0;
...
for idx1 = 1:numel(learnRate)
    for idx2 = 1:numel(numHiddenUnits1)
        for idx3 = 1:numel(numHiddenUnits2)
            
            
            % LAYERS
            layers = [
                sequenceInputLayer(numInputs)
                lstmLayer(numHiddenUnits1(idx2),'OutputMode','sequence','name','lstm1')
                lstmLayer(numHiddenUnits2(idx3),'OutputMode','last','name','lstm2')
            ...
            if Acc_all_mn < best_Acc_mean
                ...
                best_numHiddenUnits1 = numHiddenUnits1(idx2);
                best_numHiddenUnits2 = numHiddenUnits2(idx3);
            end
        end
    end
end
...
% PRINT SOME INFO
fprintf('final_Acc_mean = %.4f; final_Acc_std = %.3f; final_time = %.3f; best_learnRate = %.4f; best_numHiddenUnits1 = %d; best_numHiddenUnits2 = %d;\n', final_Acc_mean, final_Acc_std, final_time, best_learnRate, best_numHiddenUnits1, best_numHiddenUnits2);
